...
#
node.set['chef_elasticsearch']['zen_ping_enabled'] = false
...
node.set['chef_elasticsearch']['unicast_hosts'] = nodes.join(',')

# set minimum_master_nodes to n/2+1 to avoid split brain scenarios
node.default['chef_elasticsearch']['minimum_master_nodes'] = (nodes.length / 2).floor + 1
